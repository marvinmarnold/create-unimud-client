...
public class PlayerController : MonoBehaviour
{
	...
	public static bool IsPositionTableSigned()
	{
		return typeof(PositionTable).GetField("x").FieldType == typeof(long?);
	}
	...
	// TODO: Send tx
	private async UniTaskVoid SendMoveTxAsync(object x, object y)
	{
		try
		{
    ...
    void Update()
	{
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			if (IsPositionTableSigned())
			{
				SendMoveTxAsync(Convert.ToInt32(dest.x), Convert.ToInt32(dest.z)).Forget();
			}
			else
			{
				Debug.Log("moving to " + dest.x.ToString() + ", " + dest.z.ToString());
				SendMoveTxAsync(Convert.ToUInt32(dest.x), Convert.ToUInt32(dest.z)).Forget();
			}
        }
	}
